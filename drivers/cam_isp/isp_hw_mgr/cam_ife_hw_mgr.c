...
static int cam_ife_hw_mgr_event_handler_offline(
	void                                *priv,
	uint32_t                             evt_id,
	void                                *evt_info);
	...
	uint32_t *reg_base_idx)
{
	...
	struct cam_ife_hw_concrete_ctx *ctx =
		((struct cam_ife_hw_mgr_ctx *)hw_mgr_ctx)->concr_ctx;
	...
	bool user_triggered_dump)
{
	...
	if (!num_reg_dump_buf || !reg_dump_buf_desc) {
		CAM_DBG(CAM_ISP,
			...
			c_ctx->ctx_index, meta_type,
			num_reg_dump_buf, reg_dump_buf_desc);
		return rc;
	}

	if (!atomic_read(&c_ctx->cdm_done))
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (!c_ctx->init_done) {
		CAM_WARN(CAM_ISP, "c_ctx is not in init state");
		return;
	}

	/* Deinit IFE CID */
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_cid, list) {
	...
	/* Deint IFE RD */
	list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_in_rd, list) {
		cam_ife_hw_mgr_deinit_hw_res(hw_mgr_res);
	}

	/* Deinit IFE OUT */
	for (i = 0; i < CAM_IFE_HW_OUT_RES_MAX; i++)
		cam_ife_hw_mgr_deinit_hw_res(&c_ctx->res_list_ife_out[i]);

	c_ctx->init_done = false;
}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	for (i = 0; i < CAM_IFE_HW_OUT_RES_MAX; i++) {
		rc = cam_ife_hw_mgr_init_hw_res(&c_ctx->res_list_ife_out[i]);
		if (rc) {
			CAM_ERR(CAM_ISP, "Can not INIT IFE OUT (%d)",
				 c_ctx->res_list_ife_out[i].res_id);
			goto deinit;
		}
	}
	...
	uint32_t num_pd_port, uint32_t num_rdi_port, int acquire_failed)
{
	...
	hw_mgr_res = list_first_entry(&c_ctx->res_list_ife_csid,
		struct cam_ife_hw_mgr_res, list);
	...
	CAM_ERR(CAM_ISP,
		...
		(c_ctx->is_dual) ? "dual" : "single",
		hw_idx[CAM_ISP_HW_SPLIT_LEFT], hw_idx[CAM_ISP_HW_SPLIT_RIGHT],
		num_pix_port, num_pd_port, num_rdi_port, c_ctx->ctx_index);

	return;

fail:
	CAM_ERR(CAM_ISP, "Acquire HW failed for ctx:%u", c_ctx->ctx_index);
	CAM_INFO(CAM_ISP,
...
{
	struct cam_ife_hw_concrete_ctx *c_ctx = hwr_mgr_ctx->concr_ctx;
	struct cam_ife_hw_mgr_res      *hw_mgr_res = NULL;
	struct cam_ife_hw_mgr_res      *hw_mgr_res_temp = NULL;
	struct cam_isp_resource_node   *hw_res = NULL;
	...
	CAM_INFO(CAM_ISP,
		...
		hrs, min, sec, ms,
		c_ctx->ctx_index,
		(c_ctx->is_rdi_only_context ? "true" : "false"),
		(c_ctx->is_dual ? "true" : "false"));

	/* Iterate over CID resources */
	list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
		&c_ctx->res_list_ife_cid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			hw_res = hw_mgr_res->hw_res[i];
			if (hw_res && hw_res->hw_intf) {
				CAM_INFO(CAM_ISP,
	...
	struct cam_ife_hw_mgr_ctx  *ife_ctx)
{
	...
	ife_ctx->cb_priv = NULL;
	memset(ife_ctx->event_cb, 0, sizeof(ife_ctx->event_cb));

	/* ife leaf resource */
	for (i = 0; i < CAM_IFE_HW_OUT_RES_MAX; i++)
		cam_ife_hw_mgr_free_hw_res(&c_ctx->res_list_ife_out[i]);
	...
	/* ife root node */
	if (c_ctx->res_list_ife_in.res_type != CAM_IFE_HW_MGR_RES_UNINIT)
		cam_ife_hw_mgr_free_hw_res(&c_ctx->res_list_ife_in);
...
{
	int rc                              = 0;
	struct cam_ife_hw_concrete_ctx *ctx_ptr  = NULL;

	mutex_lock(&g_ife_hw_mgr.ctx_mutex);
	ctx_ptr = (*ife_ctx)->concr_ctx;
	if (ctx_ptr)
	...
	struct cam_ife_hw_mgr_ctx       **ife_ctx)
{
	...
	if (!list_empty(src_list)) {
		ctx_ptr = list_first_entry(src_list,
			struct cam_ife_hw_concrete_ctx, list);
	...
	uint32_t                         base_idx)
{
	...
	if (!c_ctx->num_base) {
		c_ctx->base[0].split_id = split_id;
		c_ctx->base[0].idx      = base_idx;
		c_ctx->num_base++;
		CAM_DBG(CAM_ISP,
			"Add split id = %d for base idx = %d num_base=%d",
			split_id, base_idx, c_ctx->num_base);
	} else {
		/*Check if base index already exists in the list */
		for (i = 0; i < c_ctx->num_base; i++) {
			if (c_ctx->base[i].idx == base_idx) {
				if (split_id != CAM_ISP_HW_SPLIT_MAX &&
					c_ctx->base[i].split_id ==
						CAM_ISP_HW_SPLIT_MAX)
					c_ctx->base[i].split_id = split_id;
		...
		if (i == c_ctx->num_base) {
			c_ctx->base[c_ctx->num_base].split_id = split_id;
			c_ctx->base[c_ctx->num_base].idx      = base_idx;
			c_ctx->num_base++;
			CAM_DBG(CAM_ISP,
	...
	struct cam_ife_hw_mgr_ctx        *ctx)
{
	...
	CAM_DBG(CAM_ISP, "ctx base num = %d", c_ctx->num_base);
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	vfe_acquire.tasklet = c_ctx->tasklet_info;

	ife_out_res = &c_ctx->res_list_ife_out[vfe_out_res_id & 0xFF];
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		if (ife_ctx->is_offline) {
			vfe_acquire.priv = c_ctx->hw_mgr;
			vfe_acquire.event_cb =
				cam_ife_hw_mgr_event_handler_offline;
		} else {
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (update_only) {
				...
				if (rc) {
					CAM_ERR(CAM_ISP,
						"Can not update out resource 0x%x",
						out_port->res_type);
					goto err;
				}
			} else {
		...
		if (!update_only) {
			...
			ife_src_res->child[ife_src_res->num_children++] =
					ife_out_res;
			CAM_DBG(CAM_ISP, "IFE SRC num_children = %d",
				ife_src_res->num_children);
		}
	...
	list_for_each_entry(ife_src_res, &c_ctx->res_list_ife_src, list) {
		...
		case CAM_ISP_HW_VFE_IN_RD:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_pixel(ife_ctx,
				ife_src_res, in_port, false, false);
			break;
		case CAM_ISP_HW_VFE_IN_LCR:
			rc = cam_ife_hw_mgr_acquire_res_ife_out_pixel(ife_ctx,
				ife_src_res, in_port, true, false);
			break;
		case CAM_ISP_HW_VFE_IN_RDI0:
		case CAM_ISP_HW_VFE_IN_RDI1:
		case CAM_ISP_HW_VFE_IN_RDI2:
	...
	uint32_t *acquired_hw_path)
{
	...
	list_for_each_entry(csid_res, &c_ctx->res_list_ife_csid, list) {
		...
		cam_ife_hw_mgr_put_res(&c_ctx->res_list_ife_src,
			&ife_src_res);
		...
		vfe_acquire.vfe_in.is_fe_enabled = c_ctx->is_fe_enabled;
		vfe_acquire.vfe_in.is_offline = c_ctx->is_offline;
		if (ife_ctx->is_offline) {
			vfe_acquire.priv = ife_hw_mgr;
			vfe_acquire.event_cb =
				cam_ife_hw_mgr_event_handler_offline;
		} else {
			vfe_acquire.priv = ife_ctx;
	...
	bool is_start_lower_idx)
{
	...
	tries = 0;
	if (csid_acquire->res_id >= CAM_IFE_PIX_PATH_RES_RDI_0 &&
		csid_acquire->res_id <= CAM_IFE_PIX_PATH_RES_RDI_3) {
		rdis[tries++] = csid_acquire->res_id;
		is_rdi = true;
	...
	for (i = CAM_IFE_CSID_HW_NUM_MAX - 1; i >= 0; i--) {
		...
		while (try_again) {
			try_again = false;
			if (is_rdi)
				csid_acquire->res_id =  rdis[tries];
			hw_intf = ife_hw_mgr->csid_devices[i];
			rc = hw_intf->hw_ops.reserve(hw_intf->hw_priv,
				csid_acquire,
				sizeof(struct
					cam_csid_hw_reserve_resource_args));
			if (is_rdi && (tries < CAM_IFE_CSID_RDI_MAX-1) &&
				rc == -EAGAIN) {
					try_again = true;
					tries++;
			} else if (!rc)
	...
	uint32_t *acquired_cnt)
{
	...
	list_for_each_entry(cid_res_iterator, &c_ctx->res_list_ife_cid,
		list) {
		tries = 0;
	...
	enum cam_ife_pix_path_res_id        *path_res_id)
{
	...
	struct cam_csid_hw_reserve_resource_args  csid_acquire;
	struct cam_ife_hw_concrete_ctx       *ife_ctx_iterator;
	...
	*cid_res = NULL;
	...
	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list, cid_res);
	...
	if (c_ctx->is_fe_enabled || in_port->usage_type)
...
acquire_successful:
	*path_res_id = csid_acquire.res_id;
	...
	c_ctx->is_dual = (bool)in_port->usage_type;
	if (c_ctx->is_dual)
		c_ctx->master_hw_idx =
	...
	cam_ife_hw_mgr_put_res(&c_ctx->res_list_ife_cid, cid_res);
	...
	cid_res_temp->parent = &c_ctx->res_list_ife_in;
	c_ctx->res_list_ife_in.child[
		c_ctx->res_list_ife_in.num_children++] = cid_res_temp;
	CAM_DBG(CAM_ISP, "IFE IN num_children = %d",
		c_ctx->res_list_ife_in.num_children);

	return 0;
put_res:
	cam_ife_hw_mgr_put_res(&c_ctx->free_res_list, cid_res);
end:
	return rc;
	...
	bool                                 crop_enable)
{
	...
	return 0;
put_res:
	cam_ife_hw_mgr_put_res(&c_ctx->free_res_list, &csid_res);
end:
	return rc;
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (c_ctx->res_list_ife_in.res_type == CAM_IFE_HW_MGR_RES_UNINIT) {
		/* first acquire */
		c_ctx->res_list_ife_in.res_type = CAM_IFE_HW_MGR_RES_ROOT;
		c_ctx->res_list_ife_in.res_id = in_port->res_type;
		c_ctx->res_list_ife_in.is_dual_vfe = in_port->usage_type;
	} else if ((c_ctx->res_list_ife_in.res_id !=
		in_port->res_type) && (!c_ctx->is_fe_enabled))  {
...
static int cam_ife_mgr_check_fe_v0(
	...
	bool *is_fe_enabled)
{
	...
	*is_fe_enabled = false;
	...
	CAM_DBG(CAM_ISP, "is_fe_enabled %d", *is_fe_enabled);

	return 0;
}

static int cam_ife_mgr_check_fe_v2(
	struct cam_isp_acquire_hw_info    *acquire_hw_info,
	uint32_t                           acquire_info_size,
	bool                              *is_fe_enabled,
	bool                              *is_offline,
	struct cam_ife_mgr_bw_data        *bw_data)
{
	...
	*is_offline = false;
	...
	CAM_DBG(CAM_ISP, "is_fe_enabled %d is_offline %d",
		*is_fe_enabled, *is_offline);

	return 0;
}

static int cam_ife_mgr_check_fe(
	struct cam_isp_acquire_hw_info    *acquire_hw_info,
	uint32_t                           acquire_info_size,
	bool                              *is_fe_enabled,
	bool                              *is_offline,
	struct cam_ife_mgr_bw_data        *bw_data)
{
	...
	case 1:
		*is_offline = false;
		return cam_ife_mgr_check_fe_v0(
	...
	int                         *lcr_count)
{
	...
	ife_hw_mgr = ife_ctx->concr_ctx->hw_mgr;
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list, &ife_bus_rd_res);
	if (rc) {
		CAM_ERR(CAM_ISP, "No more free hw mgr resource");
		goto end;
	}
	...
	if (ife_ctx->is_offline) {
		vfe_acquire.priv = ife_hw_mgr;
		vfe_acquire.event_cb = cam_ife_hw_mgr_event_handler_offline;
	} else {
		vfe_acquire.priv = ife_ctx;
		vfe_acquire.event_cb = cam_ife_hw_mgr_event_handler;
	}

	vfe_acquire.vfe_bus_rd.cdm_ops = c_ctx->cdm_ops;
	vfe_acquire.vfe_bus_rd.is_dual = (uint32_t)c_ctx->is_dual;
	vfe_acquire.vfe_bus_rd.is_offline = c_ctx->is_offline;
	...
	ife_bus_rd_res->is_dual_vfe = (uint32_t)c_ctx->is_dual;
	cam_ife_hw_mgr_put_res(&c_ctx->res_list_ife_in_rd, &ife_bus_rd_res);
...
put_res:
	cam_ife_hw_mgr_put_res(&c_ctx->free_res_list, &ife_bus_rd_res);
	...
	uint32_t                                   *acquired_hw_path)
{
	...
	ife_bus_rd_res = list_first_entry(&c_ctx->res_list_ife_in_rd,
		struct cam_ife_hw_mgr_res, list);
	...
	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list, &ife_src_res);
	if (rc) {
	...
	*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(
	...
	ife_src_res->res_type =
		(enum cam_ife_hw_mgr_res_type)vfe_acquire.rsrc_type;
	ife_src_res->res_id = vfe_acquire.vfe_in.res_id;
	ife_src_res->is_dual_vfe = (uint32_t)c_ctx->is_dual;
...
put_res:
	cam_ife_hw_mgr_put_res(&c_ctx->free_res_list, &ife_src_res);
	...
	uint32_t                            *acquired_hw_path)
{
	...
	if ((!ipp_count && !lcr_count) || !ife_rd_count) {
		CAM_ERR(CAM_ISP,
			...
			ife_rd_count, ipp_count, lcr_count);
		return -EINVAL;
	}
	...
	uint32_t *acquired_hw_path)
{
	...
	if (ipp_count || ppp_count || rdi_count) {
		...
		acquired_hw_path[0] &= 0xFFFF;
		acquired_hw_path[0] |= acq_res_id;
	...
	enum cam_cdm_cb_status status, uint64_t cookie)
{
	...
	struct cam_ife_hw_concrete_ctx *ctx = NULL;
	...
	ctx = ((struct cam_ife_hw_mgr_ctx *)
			hw_update_data->ife_mgr_ctx)->concr_ctx;
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	ife_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)acquire_args->ctxt_to_hw_map;

	/* get the ife ctx */
	rc = cam_ife_hw_mgr_get_ctx(&ife_hw_mgr->free_ctx_list, &ife_mgr_ctx);
	ife_ctx = ife_mgr_ctx->concr_ctx;
	if (rc || !ife_ctx) {
	...
	cdm_acquire.userdata = ife_mgr_ctx;
	...
	ife_mgr_ctx->in_ports = kzalloc(
			sizeof(struct cam_isp_in_port_generic_info *),
			GFP_KERNEL);
	/* acquire HW resources */
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		rc = cam_ife_mgr_acquire_get_unified_structure(acquire_hw_info,
			i, &input_size, &ife_mgr_ctx->in_ports[i]);
		if (rc < 0) {
			CAM_ERR(CAM_ISP, "Failed in parsing: %d", rc);
			goto free_res;
		}
		CAM_DBG(CAM_ISP, "in_res_type %x",
				ife_mgr_ctx->in_ports[i]->res_type);

		if ((ife_mgr_ctx->in_ports[i]->cust_node) &&
				(!ife_ctx->custom_enabled)) {
		...
		if (ife_ctx->is_offline)
			rc = cam_ife_mgr_acquire_hw_for_offline_ctx(
				ife_mgr_ctx, ife_mgr_ctx->in_ports[i],
				&num_pix_port_per_in,
		...
		if (rc) {
			cam_ife_hw_mgr_print_acquire_info(ife_mgr_ctx,
	...
	ife_mgr_ctx->concr_ctx->acquired_hw_id =
		acquire_args->acquired_hw_id[0];
	...
	/* Process base info */
	rc = cam_ife_mgr_process_base_info(ife_mgr_ctx);
	if (rc) {
	...
	cam_ife_hw_mgr_put_ctx(&ife_hw_mgr->used_ctx_list, &ife_mgr_ctx);

	return 0;
free_mem:
	for (i = 0; i < ife_mgr_ctx->num_in_ports; i++) {
		kfree(ife_mgr_ctx->in_ports[i]->data);
		kfree(ife_mgr_ctx->in_ports[i]);
	}
	kfree(ife_mgr_ctx->in_ports);
free_res:
	cam_ife_hw_mgr_release_hw_for_ctx(ife_mgr_ctx);
free_cdm:
	cam_cdm_release(ife_ctx->cdm_handle);
free_ctx:
	cam_ife_hw_mgr_put_ctx(&ife_hw_mgr->free_ctx_list, &ife_mgr_ctx);
err:
	CAM_DBG(CAM_ISP, "Exit...(rc=%d)", rc);
	return rc;
}
...
static int cam_ife_mgr_acquire_dev(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	ife_mgr_ctx->cb_priv = acquire_args->context_data;
	for (i = 0; i < CAM_ISP_HW_EVENT_MAX; i++)
		ife_mgr_ctx->event_cb[i] = acquire_args->event_cb;
	...
	cdm_acquire.cell_index = 0;
	cdm_acquire.handle = 0;
	cdm_acquire.userdata = ife_hw_mgr;
	cdm_acquire.base_array_cnt = CAM_IFE_HW_NUM_MAX;
	for (i = 0, j = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		if (ife_hw_mgr->cdm_reg_map[i])
			cdm_acquire.base_array[j++] =
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		if (!IS_ERR(in_port)) {
			...
			rc = cam_ife_mgr_acquire_hw_for_ctx(ife_mgr_ctx,
					...
					void *config_hw_args)
{
	...
	cfg = config_hw_args;
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)cfg->ctxt_to_hw_map;
	if (!hw_mgr_ctx) {
	...
	if (!c_ctx) {
		CAM_ERR(CAM_ISP, "c_ctx is NULL");
		return -EPERM;
	}
	if (!c_ctx->ctx_in_use || !c_ctx->cdm_cmd) {
		CAM_ERR(CAM_ISP,
			"Invalid context parameters : ctx_in_use=%d, cdm_cmd=%pK",
			c_ctx->ctx_in_use, c_ctx->cdm_cmd);
		return -EPERM;
	}
	...
	if (atomic_read(&c_ctx->overflow_pending)) {
		CAM_DBG(CAM_ISP,
			...
			c_ctx, c_ctx->ctx_index, cfg->request_id);
		return -EPERM;
	}

	hw_update_data = (struct cam_isp_prepare_hw_update_data  *) cfg->priv;
	if (!hw_update_data) {
		CAM_ERR(CAM_ISP, "hw_update_data is NULL");
		return -EPERM;
	}
	hw_update_data->ife_mgr_ctx = hw_mgr_ctx;
	...
	CAM_DBG(CAM_ISP,
		"Enter c_ctx id:%d num_hw_upd_entries %d request id: %llu",
		c_ctx->ctx_index, cfg->num_hw_update_entries, cfg->request_id);
	if (cfg->num_hw_update_entries > 0) {
		cdm_cmd = c_ctx->cdm_cmd;
		if (!cdm_cmd) {
			CAM_ERR(CAM_ISP, "ctx_cmd is NULL");
			return -EPERM;
		}
		...
		if (!cmd) {
			CAM_ERR(CAM_ISP, "cmd[%d] is NULL", i);
			return -EPERM;
		}
			if (cfg->reapply &&
		...
		reinit_completion(&c_ctx->config_done_complete);
		c_ctx->applied_req_id = cfg->request_id;

		CAM_DBG(CAM_ISP, "Submit to CDM %d, skip %d",
					cdm_cmd->cmd_arrary_count, skip);
		atomic_set(&c_ctx->cdm_done, 0);
		rc = cam_cdm_submit_bls(c_ctx->cdm_handle, cdm_cmd);
		if (rc) {
			CAM_ERR(CAM_ISP,
		...
		for (i = 0; i < CAM_IFE_HW_CONFIG_WAIT_MAX_TRY; i++) {
			...
			if (rem_jiffies == 0) {
				...
				CAM_ERR(CAM_ISP,
					...
					cfg->request_id, c_ctx->ctx_index);
				rc = -ETIMEDOUT;
				goto end;
			} else {
				rc = 0;
...
static int cam_ife_mgr_stop_hw_in_overflow(void *stop_hw_args)
{
	...
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)stop_args->ctxt_to_hw_map;
	if (!hw_mgr_ctx) {
		CAM_ERR(CAM_ISP, "Invalid context is used");
		return -EPERM;
	}
	...
	cam_ife_mgr_csid_stop_hw(hw_mgr_ctx, &ctx->res_list_ife_cid,
	...
	cam_ife_mgr_csid_stop_hw(hw_mgr_ctx, &ctx->res_list_ife_csid,
		master_base_idx, CAM_CSID_HALT_IMMEDIATELY);
	...
	cam_tasklet_stop(ctx->tasklet_info);
	...
	enum cam_vfe_bw_control_action action)
{
	...
	CAM_DBG(CAM_ISP, "Enter...ctx id:%d", c_ctx->ctx_index);
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	cam_ife_mgr_pause_hw(hw_mgr_ctx);
...
static int cam_ife_mgr_restart_hw(void *start_hw_args)
{
	...
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)start_args->ctxt_to_hw_map;
	ctx = hw_mgr_ctx->concr_ctx;
	if (!ctx || !ctx->ctx_in_use) {
		CAM_ERR(CAM_ISP, "Invalid context is used");
		return -EPERM;
	...
	cam_tasklet_start(ctx->tasklet_info);

	/* start the IFE out devices */
	for (i = 0; i < CAM_IFE_HW_OUT_RES_MAX; i++) {
		rc = cam_ife_hw_mgr_start_hw_res(
			&ctx->res_list_ife_out[i], hw_mgr_ctx);
		if (rc) {
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_in_rd, list) {
		rc = cam_ife_hw_mgr_start_hw_res(hw_mgr_res, hw_mgr_ctx);
		if (rc) {
...
static int cam_ife_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	rc = cam_ife_hw_mgr_init_hw(hw_mgr_ctx);
	if (rc) {
...
static int cam_ife_mgr_reset(void *hw_mgr_priv, void *hw_reset_args)
{
	...
	ctx = ((struct cam_ife_hw_mgr_ctx *)
				reset_args->ctxt_to_hw_map)->concr_ctx;
	if (!ctx || !ctx->ctx_in_use) {
		CAM_ERR(CAM_ISP, "Invalid context is used");
		return -EPERM;
	}
					...
					void *release_hw_args)
{
	...
	cam_ife_hw_mgr_release_hw_for_ctx(hw_mgr_ctx);
	...
	cam_ife_hw_mgr_put_ctx(&hw_mgr->free_ctx_list, &hw_mgr_ctx);
	return rc;
}
	...
	uint32_t blob_type, uint32_t blob_size, uint8_t *blob_data)
{
	...
	ife_ctx = ((struct cam_ife_hw_mgr_ctx *)
			prepare->ctxt_to_hw_map)->concr_ctx;
	ife_hw_mgr = ife_ctx->hw_mgr;
	CAM_DBG(CAM_ISP, "Context[%pK][%d] blob_type=%d, blob_size=%d",
		ife_ctx, ife_ctx->ctx_index, blob_type, blob_size);
	...
	if (ife_ctx->is_offline && ife_ctx->offline_clk !=
					ife_hw_mgr->offline_clk) {
		struct cam_isp_clock_config    offline_clk_cfg;

		ife_ctx->offline_clk = ife_hw_mgr->offline_clk;
		offline_clk_cfg.num_rdi = 0;
		offline_clk_cfg.left_pix_hz = ife_hw_mgr->offline_clk;
		CAM_DBG(CAM_ISP, "Override offline IFE clk to %u",
				offline_clk_cfg.left_pix_hz);
		rc = cam_isp_blob_clock_update(blob_type, blob_info,
			&offline_clk_cfg, ife_ctx);
		if (rc)
			CAM_ERR(CAM_PERF, "Clock Update Failed, rc=%d", rc);
	}
	return rc;
	...
	void *prepare_hw_update_args)
{
	...
	if ((((prepare->packet->header.op_code + 1) & 0xF) ==
		CAM_ISP_PACKET_INIT_DEV) && (!ctx->is_offline ||
...
static unsigned int cam_ife_mgr_flush_in_queue(
		struct cam_ife_hw_mgr *ife_hw_mgr,
		uint32_t ctx_idx, bool just_incomplete, int64_t req_id)
{
...
static int cam_ife_mgr_flush_ctx(void *hw_mgr_priv,
		struct cam_ife_hw_mgr_ctx *hw_mgr_ctx, int64_t req_id)
{
	struct cam_ife_hw_mgr *ife_hw_mgr        = hw_mgr_priv;
	mutex_lock(&ife_hw_mgr->ctx_mutex);
	cam_ife_mgr_flush_in_queue(ife_hw_mgr, hw_mgr_ctx->ctx_idx, false,
			req_id);
	mutex_unlock(&ife_hw_mgr->ctx_mutex);
	return 0;
}
static int cam_ife_mgr_int_cmd(void                        *hw_mgr_priv,
				struct cam_ife_hw_mgr_ctx  *hw_mgr_ctx,
				struct cam_isp_hw_cmd_args *isp_hw_cmd_args)
{
	...
	case CAM_ISP_HW_MGR_CMD_CHECK_START_OFFLINE:
		if (!hw_mgr_ctx->concr_ctx) {
			CAM_ERR(CAM_ISP, "Invalid context");
			return 0;
		}
		cam_ife_mgr_check_start_processing(hw_mgr_priv, hw_mgr_ctx);
		break;
	case CAM_ISP_HW_MGR_CMD_FLUSH_OFFLINE:
		cam_ife_mgr_flush_ctx(hw_mgr_priv, hw_mgr_ctx,
				isp_hw_cmd_args->u.req_id);
		break;
	case CAM_ISP_HW_MGR_CMD_PAUSE_HW:
		cam_ife_mgr_pause_hw(hw_mgr_ctx);
		break;
	case CAM_ISP_HW_MGR_CMD_RESUME_HW:
		cam_ife_mgr_resume_hw(hw_mgr_ctx);
		break;
	case CAM_ISP_HW_MGR_CMD_SOF_DEBUG:
		cam_ife_mgr_sof_irq_debug(hw_mgr_ctx,
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	struct cam_ife_hw_mgr  *hw_mgr = hw_mgr_priv;
	struct cam_ife_hw_mgr_ctx *hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		...
		rc = cam_ife_mgr_int_cmd(hw_mgr_priv, hw_mgr_ctx,
					isp_hw_cmd_args);
		break;
	case CAM_HW_MGR_CMD_DUMP_PF_INFO:
		...
		for (i = 0; i < CAM_IFE_HW_OUT_RES_MAX; i++) {
			...
			cam_ife_hw_mgr_handle_hw_dump_bus_info(hw_mgr_ctx,
				&event_info);
		}
		break;
	case CAM_HW_MGR_CMD_REG_DUMP_ON_FLUSH:
		...
		rc = cam_ife_mgr_handle_reg_dump(hw_mgr_ctx,
			ctx->reg_dump_buf_desc,
	...
	case CAM_HW_MGR_CMD_DUMP_ACQ_INFO:
		cam_ife_hw_mgr_dump_acq_data(hw_mgr_ctx);
		break;
	default:
		...
		struct cam_hw_dump_args *dump_args)
{
	...
	rc = cam_ife_mgr_handle_reg_dump(ife_ctx,
		ife_ctx->concr_ctx->reg_dump_buf_desc,
		ife_ctx->concr_ctx->num_reg_dump_buf,
...
static int cam_ife_mgr_dump(void *hw_mgr_priv, void *args)
{
	...
	if (ife_ctx->num_reg_dump_buf) {
		cam_ife_mgr_user_dump_hw(hw_mgr_ctx, dump_args);
		goto end;
	}
	...
	uint64_t                             *boot_time_stamp)
{
	...
	hw_mgr_res = list_first_entry(&ife_ctx->concr_ctx->res_list_ife_csid,
		struct cam_ife_hw_mgr_res, list);
...
static int cam_ife_mgr_process_recovery_cb(void *priv, void *data)
{
	...
	case CAM_ISP_HW_ERROR_BUSIF_OVERFLOW:
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			...
			((struct cam_ife_hw_mgr_ctx *)
				start_args.ctxt_to_hw_map)->concr_ctx = ctx;
	...
	uint32_t                     size)
{
	...
	bool                  rc = false;
	uint32_t              i = 0, j = 0;
	uint32_t              max_idx =  ife_ctx->num_base;
	...
	for (i = 0; i < max_idx; i++) {
		if (affected_core[ife_ctx->base[i].idx])
			rc = true;
		else {
			ctx_affected_core_idx[j] = ife_ctx->base[i].idx;
			j = j + 1;
		}
	}
	...
	struct cam_hw_event_recovery_data     *recovery_data)
{
	...
	struct cam_ife_hw_concrete_ctx *ife_hwr_mgr_ctx = NULL;
	struct cam_ife_hw_mgr_ctx      *ife_ctx = NULL;
...
skip_overflow:
		...
		idx = ife_hwr_mgr_ctx->served_ctx_id[
					ife_hwr_mgr_ctx->served_ctx_w];
		if (!ife_hwr_mgr->virt_ctx_pool[idx].ctx_in_use) {
			CAM_ERR(CAM_ISP, "Invalid context");
			return -EINVAL;
		}
		ife_ctx = &ife_hwr_mgr->virt_ctx_pool[idx];
		notify_err_cb = ife_ctx->event_cb[event_type];
	...
	void                                 *evt_info)
{
	...
	hw_mgr_res =
		&c_ctx->res_list_ife_out[out_port_id];
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		if (!hw_mgr_res->hw_res[i])
			continue;
		hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
...
static int cam_ife_hw_mgr_handle_hw_dump_info(
	...
	void                                 *evt_info)
{
	struct cam_ife_hw_concrete_ctx     *ife_hw_mgr_ctx =
		((struct cam_ife_hw_mgr_ctx *)ctx)->concr_ctx;
	struct cam_isp_hw_event_info  *event_info =
		(struct cam_isp_hw_event_info *)evt_info;
	struct cam_ife_hw_mgr_res     *hw_mgr_res = NULL;
	struct cam_isp_resource_node  *rsrc_node = NULL;
	...
	void                                    *evt_info)
{
	...
	if (!hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_REG_UPDATE]) {
	...
	ife_hwr_irq_rup_cb =
		hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_REG_UPDATE];
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		...
		ife_hwr_irq_rup_cb(hw_mgr_ctx->cb_priv,
	...
	uint32_t                              hw_event_type)
{
	...
	if (!ife_ctx->concr_ctx->is_dual)
		return 0;

	switch (hw_event_type) {
	case CAM_ISP_HW_EVENT_SOF:
		event_cnt = ife_ctx->concr_ctx->sof_cnt;
		break;
	case CAM_ISP_HW_EVENT_EPOCH:
		event_cnt = ife_ctx->concr_ctx->epoch_cnt;
		break;
	case CAM_ISP_HW_EVENT_EOF:
		event_cnt = ife_ctx->concr_ctx->eof_cnt;
		break;
	default:
		return 0;
	}

	master_hw_idx = ife_ctx->concr_ctx->master_hw_idx;
	slave_hw_idx =  ife_ctx->concr_ctx->slave_hw_idx;
	...
	if ((event_cnt[master_hw_idx] &&
		...
		if (ife_ctx->concr_ctx->dual_ife_irq_mismatch_cnt > 10) {
			rc = -1;
			return rc;
		}
		...
		if (event_cnt[master_hw_idx] >= 2) {
			...
			ife_ctx->concr_ctx->dual_ife_irq_mismatch_cnt++;
		}
		...
		if (ife_ctx->concr_ctx->dual_ife_irq_mismatch_cnt == 1)
			cam_ife_mgr_ctx_irq_dump(ife_ctx);
		rc = 0;
	}
	...
	void                                 *evt_info)
{
	...
	if (!hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EPOCH]) {
	...
	ife_hw_irq_epoch_cb =
		hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EPOCH];

	switch (event_info->res_id) {
	case CAM_ISP_HW_VFE_IN_CAMIF:
		ife_ctx->epoch_cnt[event_info->hw_idx]++;
		rc = cam_ife_hw_mgr_check_irq_for_dual_vfe(hw_mgr_ctx,
			CAM_ISP_HW_EVENT_EPOCH);
		if (!rc) {
			epoch_done_event_data.frame_id_meta =
				event_info->th_reg_val;
			ife_hw_irq_epoch_cb(hw_mgr_ctx->cb_priv,
	...
	void                                 *evt_info)
{
	...
	if (!hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_SOF]) {
	...
	ife_hw_irq_sof_cb =
		hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_SOF];

	switch (event_info->res_id) {
	case CAM_ISP_HW_VFE_IN_CAMIF:
	case CAM_ISP_HW_VFE_IN_RD:
		ife_ctx->sof_cnt[event_info->hw_idx]++;
		rc = cam_ife_hw_mgr_check_irq_for_dual_vfe(hw_mgr_ctx,
		...
		if (!rc) {
			...
			if (ife_ctx->use_frame_header_ts)
	...
	void                                 *evt_info)
{
	...
	if (!hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EOF]) {
	...
	ife_hw_irq_eof_cb =
		hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EOF];

	switch (event_info->res_id) {
	case CAM_ISP_HW_VFE_IN_CAMIF:
		ife_ctx->eof_cnt[event_info->hw_idx]++;
		rc = cam_ife_hw_mgr_check_irq_for_dual_vfe(hw_mgr_ctx,
			CAM_ISP_HW_EVENT_EOF);
		if (!rc) {
			ife_hw_irq_eof_cb(hw_mgr_ctx->cb_priv,
	...
	void                                *evt_info)
{
	...
	if (!hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_DONE]) {
		CAM_ERR(CAM_ISP, "event_cb[CAM_ISP_HW_EVENT_DONE] is null");
		return 0;
	}

	ife_hwr_irq_wm_done_cb =
		hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_DONE];
	...
	if (buf_done_event_data.num_handles > 0 && ife_hwr_irq_wm_done_cb) {
		...
		ife_hwr_irq_wm_done_cb(hw_mgr_ctx->cb_priv,
	...
	void                                *evt_info)
{
	...
	if (!ife_hw_mgr) {
		CAM_ERR(CAM_ISP, "Invalid HW mgr!");
		return -EINVAL;
	}

	if (!event_info) {
		CAM_ERR(CAM_ISP, "Invalid event info");
		return -EINVAL;
	}

	hw_num = cam_convert_hw_idx_to_ife_hw_num(event_info->hw_idx);
	list_for_each_entry(ife_ctx, &ife_hw_mgr->used_ctx_list, list) {
		if (ife_ctx->acquired_hw_id == hw_num)
			break;
	}

	if (!ife_ctx) {
		CAM_ERR(CAM_ISP, "Invalid ife_ctx!");
		return -EINVAL;
	}
	switch (evt_id) {
	case CAM_ISP_HW_EVENT_EPOCH:
		ife_ctx->served_ctx_r = ife_ctx->served_ctx_w;
		ctx_idx_rd = ife_ctx->served_ctx_w;
		break;
	case CAM_ISP_HW_EVENT_SOF:
	case CAM_ISP_HW_EVENT_REG_UPDATE:
		ctx_idx_rd = ife_ctx->served_ctx_w;
		break;
	case CAM_ISP_HW_EVENT_EOF:
		ctx_idx_rd = ife_ctx->served_ctx_r;
		atomic_set(&ife_ctx->ctx_state, CAM_IFE_HW_STATE_STARTED);
		break;
	default:
		ctx_idx_rd = ife_ctx->served_ctx_r;
		break;
	...
	idx = ife_ctx->served_ctx_id[ctx_idx_rd];
	if (!ife_hw_mgr->virt_ctx_pool[idx].ctx_in_use) {
		CAM_ERR(CAM_ISP, "Invalid context");
		return -EINVAL;
	}
	hw_mgr_ctx = &ife_hw_mgr->virt_ctx_pool[idx];

	rc = cam_ife_hw_mgr_event_handler((void *)hw_mgr_ctx, evt_id, evt_info);
	return rc;
}
...
static int cam_ife_mgr_free_in_proc_req(struct cam_ife_hw_mgr *ife_hw_mgr,
					uint32_t hw_id)
{
	struct cam_ife_mgr_offline_in_queue *c_elem;
	struct cam_ife_mgr_offline_in_queue *c_elem_temp;
...
static int cam_ife_mgr_enqueue_offline_config(void *hw_mgr_priv,
	void *config_hw_args)
{
	int rc = -EINVAL;
	struct cam_ife_mgr_offline_in_queue *c_elem;
	...
	list_for_each_entry(c_elem, &ife_hw_mgr->input_queue.list, list) {
		if (c_elem->prepare.packet->header.request_id !=
				cfg->request_id ||
			c_elem->ctx_idx != hw_mgr_ctx->ctx_idx)
			continue;
		rc = 0;
		...
		struct cam_ife_hw_mgr_ctx *hw_mgr_ctx)
{
	...
	list_for_each_entry(ife_ctx, &ife_hw_mgr->used_ctx_list, list) {
		state = atomic_read(&ife_ctx->ctx_state);
		if (!ife_ctx->is_offline ||
...
/* -------------------------------------------------------------------------- */
static int cam_ife_mgr_v_get_hw_caps(void *hw_mgr_priv, void *hw_caps_args)
{
	return cam_ife_mgr_get_hw_caps(hw_mgr_priv, hw_caps_args);
}
...
static uint32_t cam_ife_mgr_required_offline_hw(void *hw_mgr_priv, bool stop)
{
	struct cam_ife_hw_mgr          *ife_hw_mgr = hw_mgr_priv;
	...
	if (total_bw / req_hw > max_bw)
		...
		ife_hw_mgr->offline_clk = cam_ife_mgr_bw_to_clk(max_bw,
				ife_hw_mgr->bytes_per_clk);
	...
	CAM_DBG(CAM_ISP,
			"Offline starting %d, CTXs %d, BW: %u needed %d IFEs @ :clk %d",
			stop, cnt, total_bw,
			req_hw, ife_hw_mgr->offline_clk);
	return req_hw;
}

static int cam_ife_mgr_v_acquire(void *hw_mgr_priv, void *acquire_hw_args)
{
	struct cam_hw_acquire_args     *acq_args_ptr =
		(struct cam_hw_acquire_args *)acquire_hw_args;
	struct cam_ife_hw_mgr_ctx      *ife_mgr_ctx;
	struct cam_isp_acquire_hw_info *acquire_hw_info =
		(struct cam_isp_acquire_hw_info *)acq_args_ptr->acquire_info;
	struct cam_ife_hw_mgr          *ife_hw_mgr = hw_mgr_priv;
	struct cam_ife_offline_hw      *acquired_hw_data;
	...
	for (i = 0; i < CAM_ISP_HW_EVENT_MAX; i++)
		ife_mgr_ctx->event_cb[i] = acq_args_ptr->event_cb;
	...
	if (is_offline) {
		...
		if (ctx_idx <
			...
			if (rc) {
				if (!ctx_idx) {
					/* There is no available HW to use */
					ife_mgr_ctx->ctx_in_use = true;
					return rc;
				}
				ctx_idx--;
		...
		if (allocated) {
			...
			acquired_hw_data =
				&ife_hw_mgr->acquired_hw_pool[ctx_idx];
			acquired_hw_data->ife_ctx =
				ife_mgr_ctx->concr_ctx;
			acquired_hw_data->ife_ctx->is_offline = true;
			...
			acquired_hw_data->custom_enabled =
				acq_args_ptr->custom_enabled;
			acquired_hw_data->use_frame_header_ts =
				acq_args_ptr->use_frame_header_ts;
			acquired_hw_data->valid_acquired_hw =
				acq_args_ptr->valid_acquired_hw;
			memcpy(&acquired_hw_data->acquired_hw_id,
				acq_args_ptr->acquired_hw_id,
				sizeof(acq_args_ptr->acquired_hw_id));
			memcpy(
			    ife_hw_mgr->acquired_hw_pool[0].acquired_hw_path,
			 ...
			 */
			acquired_hw_data = &ife_hw_mgr->acquired_hw_pool[0];
			ife_mgr_ctx->concr_ctx = acquired_hw_data->ife_ctx;
			cam_ife_mgr_acquire_offline_out(ife_mgr_ctx,
				acq_args_ptr);
			ife_mgr_ctx->hw_mgr = acquired_hw_data->ife_ctx->hw_mgr;
			acq_args_ptr->custom_enabled =
				acquired_hw_data->custom_enabled;
			acq_args_ptr->use_frame_header_ts =
				acquired_hw_data->use_frame_header_ts;
			acq_args_ptr->valid_acquired_hw =
				acquired_hw_data->valid_acquired_hw;
			/* Should not be used for offline */
			memcpy(acq_args_ptr->acquired_hw_id,
				&acquired_hw_data->acquired_hw_id,
				sizeof(acq_args_ptr->acquired_hw_id));
			memcpy(
				acq_args_ptr->acquired_hw_path,
...
static int cam_ife_mgr_v_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	struct cam_isp_start_args        *start_isp = start_hw_args;
	struct cam_ife_hw_mgr            *ife_hw_mgr = hw_mgr_priv;
	struct cam_ife_hw_mgr_ctx        *hw_mgr_ctx;
	...
	if (hw_mgr_ctx->is_offline) {
		...
		for (i = 0; i < num_ctx; i++) {
			c_ctx = ife_hw_mgr->acquired_hw_pool[i].ife_ctx;
			if (c_ctx && atomic_read(&c_ctx->ctx_state) ==
						CAM_IFE_HW_STATE_STOPPED) {
				hw_mgr_ctx->concr_ctx = c_ctx;
				atomic_set(&hw_mgr_ctx->concr_ctx->ctx_state,
						CAM_IFE_HW_STATE_STARTING);
				break;
			}
		}
...
static int cam_ife_mgr_v_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	if (hw_mgr_ctx->is_offline) {
		...
		/* For multiple offline camera, stop HW only if there are no
		 * other contexts that can use it. Need to enhance for more
		...
		if (num_ctx <=
			...
			if (atomic_read(&ife_ctx->ctx_state) ==
						CAM_IFE_HW_STATE_BUSY)
...
static int cam_ife_mgr_v_release_hw(void *hw_mgr_priv, void *release_hw_args)
{
	struct cam_ife_hw_mgr_ctx   *hw_mgr_ctx;
	struct cam_ife_hw_mgr       *ife_hw_mgr = hw_mgr_priv;
	struct cam_hw_release_args  *release_args = release_hw_args;
...
static int cam_ife_mgr_v_prepare_hw_update(void *hw_mgr_priv,
	void *prepare_hw_update_args)
{
	struct cam_hw_prepare_update_args *prepare =
		(struct cam_hw_prepare_update_args *) prepare_hw_update_args;
	...
	if (!hw_mgr_ctx->is_offline)
		...
		rc = cam_ife_mgr_enqueue_offline_update(hw_mgr_priv,
				prepare_hw_update_args);
		rc = cam_isp_count_hw_entries(prepare);
	}
	return rc;
}
static int cam_ife_mgr_v_config_hw(void *hw_mgr_priv, void *config_hw_args)
{
	...
	cfg = (struct cam_hw_config_args *)config_hw_args;
	if (!cfg || !cfg->ctxt_to_hw_map)
		return -EINVAL;
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *) cfg->ctxt_to_hw_map;
	if (!hw_mgr_ctx->is_offline)
		return cam_ife_mgr_config_hw(hw_mgr_priv, config_hw_args);
...
{
	return cam_ife_mgr_cmd(hw_mgr_priv, cmd_args);
}
static int cam_ife_mgr_v_reset(void *hw_mgr_priv, void *hw_reset_args)
{
	return cam_ife_mgr_reset(hw_mgr_priv, hw_reset_args);
}
static int cam_ife_mgr_v_dump(void *hw_mgr_priv, void *args)
{
	return cam_ife_mgr_dump(hw_mgr_priv, args);
}
/* -------------------------------------------------------------------------- */
...
int cam_ife_hw_mgr_init(struct cam_hw_mgr_intf *hw_mgr_intf, int *iommu_hdl)
{
	...
	for (i = 0, j = 0; i < CAM_IFE_HW_NUM_MAX; i++) {
		...
		if (!rc) {
			...
			if (!g_ife_hw_mgr.ife_devices[i]->hw_ops.process_cmd(
					vfe_hw,
	...
	INIT_LIST_HEAD(&g_ife_hw_mgr.input_queue.list);
	...
	for (i = 0; i < CAM_CTX_MAX; i++) {
		...
		g_ife_hw_mgr.ctx_pool[i].tasklet_info =
	...
	hw_mgr_intf->hw_get_caps = cam_ife_mgr_v_get_hw_caps;
	hw_mgr_intf->hw_acquire = cam_ife_mgr_v_acquire;
	hw_mgr_intf->hw_start = cam_ife_mgr_v_start_hw;
	hw_mgr_intf->hw_stop = cam_ife_mgr_v_stop_hw;
	hw_mgr_intf->hw_read = cam_ife_mgr_read;
	hw_mgr_intf->hw_write = cam_ife_mgr_write;
	hw_mgr_intf->hw_release = cam_ife_mgr_v_release_hw;
	hw_mgr_intf->hw_prepare_update = cam_ife_mgr_v_prepare_hw_update;
	hw_mgr_intf->hw_config = cam_ife_mgr_v_config_hw;
	hw_mgr_intf->hw_cmd = cam_ife_mgr_v_cmd;
	hw_mgr_intf->hw_reset = cam_ife_mgr_v_reset;
	hw_mgr_intf->hw_dump = cam_ife_mgr_v_dump;
	...
	if (rc) {
		for (i = 0; i < CAM_CTX_MAX; i++) {
			...
			g_ife_hw_mgr.ctx_pool[i].tasklet_info = NULL;
		}
	}
